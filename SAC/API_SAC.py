...
def insert_data(connection, data_row):
    ...
    [Nota_da_avaliacao], [Demanda], [Plano_ação], [Número_da_OS], [Cliente_Reincidente], [Número_NFD]
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 
    ?, ?, ?, ?, ?, ?, ?)
    """
...
def fetch_all_tickets(url, auth, headers, connection):
    while url:
...
def job():
    ...
    fetch_all_tickets(url, auth, headers, connection)
